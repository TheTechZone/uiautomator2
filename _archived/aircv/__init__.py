...
class Aircv(object):
    ...
    def detection_screen(self):
        ...
        if (
            ...
            and display_width / display_height != 16 / 9
        ):
            raise RuntimeError(
                "Does not support current mobile phones, The screen ratio is not 16:9"
            )
    ...
    def get_scaling_ratio(self):
        ...
        while True:
            if self.aircv_cache_image is not None:
                self.zoom_out = (
                    1.0 * self.d.info["displayHeight"] / self.aircv_cache_image.shape[0]
                )
                break
    ...
    def start_get_screen(self):
        ...
        def on_message(ws, message):
            ...
            if isinstance(message, bytes):
                ...
                if (
                    int(time.time()) - this.__rcv_interva_time_cache
                    >= Aircv.rcv_interval
                ):
        ...
        if not self.ws_screen or not self.ws_screen.keep_running:
            self.ws_screen = websocket.WebSocketApp(
                "ws://" + self.d._host + ":" + str(self.d._port) + "/minicap",
                on_open=on_open,
    ...
    def find_template_by_crop(self, img, area=None):
        ...
        return (
            (int(point[0] * self.zoom_out), int(point[1] * self.zoom_out))
            if point
            else None
        )
    ...
    def click_index(self, img, index=1, maxcnt=20, timeout=timeout):
        ...
        while timeout:
            if self.debug:
                print(timeout)
            if self.aircv_cache_image is not None:
                result_list = self.cvHandler.find_all_template(
                    self.aircv_cache_image, img_serch, maxcnt=maxcnt
                )
    ...
    def swipe(
        self, img_from, img_to, duration=0.1, steps=None, timeout=timeout, area=None
    ):
        point_from = None
        point_to = None
        while timeout:
