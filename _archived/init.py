...
from uiautomator2.version import (
    __apk_version__,
...
def mirror_download(url: str, filename=None):
    ...
    if url.startswith(github_host):
        mirror_url = (
            "https://tool.appetizer.io" + url[len(github_host) :]
        )  # mirror of github
        try:
            return cache_download(
                mirror_url, filename, timeout=60, storepath=storepath, logger=logger
            )
...
class Initer:
    ...
    def atx_agent_url(self):
        ...
        return GITHUB_BASEURL + "/atx-agent/releases/download/%s/%s" % (
            __atx_agent_version__,
            name.format(v=__atx_agent_version__),
        )
    ...
    def push_url(
        self, url, dest=None, mode=0o755, tgz=False, extract_name=None
    ):  # yapf: disable
    ...
    def is_apk_outdated(self):
        ...
        if apk_debug["signature"] != apk_debug_test["signature"]:
            ...
            if (
                abs(
                    apk_debug["first_install_time"]
                    - apk_debug_test["first_install_time"]
                )
                > max_delta
            ):
                logger.debug(
    ...
    def is_atx_agent_outdated(self):
        ...
        logger.debug("Real version: %s, Expect version: %s", real_ver, want_ver)
    ...
    def check_atx_agent_version(self):
        ...
        version = requests.get(
            "http://%s:%d/version" % (self._device._client.host, port)
        ).text.strip()
        logger.debug("atx-agent version %s", version)

        wlan_ip = requests.get(
            "http://%s:%d/wlan/ip" % (self._device._client.host, port)
        ).text.strip()
    ...
    def install(self):
        ...
        if self.abi == "x86":
            logger.info("abi:x86 not supported well, skip install minicap")
