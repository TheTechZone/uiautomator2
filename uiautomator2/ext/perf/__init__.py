...
_NetStats = namedtuple(
    ...
    tx_tcp_bytes tx_tcp_packets tx_udp_bytes tx_udp_packets tx_other_bytes tx_other_packets""".split(),
)
...
class Perf(object):
    ...
    def _cpu_rawdata_collect(self, pid):
        ...
        proc_stat = self.shell(["cat", "/proc/%d/stat" % pid]).output.split(") ")
        pjiff = 0
        if len(proc_stat) > 1:
    ...
    def cpu(self, pid):
        ...
        # second time jiffies
        self._data[store_key] = tjiff2, idle2, pjiff2 = self._cpu_rawdata_collect(pid)
        ...
        scpu = (
            100.0 * ((tjiff2 - idle2) - (tjiff1 - idle1)) / (tjiff2 - tjiff1)
        )  # system cpu
    ...
    def netstat(self, pid):
        ...
        """
        m = re.search(
            r"^Uid:\s+(\d+)", self.shell(["cat", "/proc/%d/status" % pid]).output, re.M
        )
        if not m:
            return (0, 0, 0, 0, 0, 0)
        uid = m.group(1)
        lines = self.shell(["cat", "/proc/net/xt_qtaguid/stats"]).output.splitlines()
    ...
    def _current_view(self, app=None):
        d = self.d
        views = self.shell(["dumpsys", "SurfaceFlinger", "--list"]).output.splitlines()
        if not app:
            app = d.app_current()
    ...
    def _dump_surfaceflinger(self, view):
        ...
        for line in self.shell(
            ["dumpsys", "SurfaceFlinger", "--latency", view]
        ).output.splitlines():
            fields = line.split()
            if len(fields) != 3:
                continue
    ...
    def csv2images(self, src=None, target_dir="."):
        ...
        plt.legend()
        plt.title(
            "\n".join(["Network", timestr, "Recv %s, Send %s" % (rx_str, tx_str)]),
            loc="left",
        )
